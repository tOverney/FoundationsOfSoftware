...
object Infer {
  ...
  private def unify(cs: List[Constraint], init: Substitution): Substitution =
    (cs foldLeft init) {
      case (subst, (tp1, tp2)) if tp1 == tp2 =>
        subst

      case (subst, (tp1: TypeVar, tp2)) =>
        unify(tp1, tp2, subst)

      case (subst, (tp1, tp2: TypeVar)) =>
        unify(tp2, tp1, subst)

      case (subst, (FunType(from1, to1), FunType(from2, to2))) =>
        unify(List((from1, from2), (to1, to2)), subst)

      case (_, (tp1, tp2)) =>
        throw new TypeError(s"Could not unify: $tp1 with $tp2")
    }

  private def unify(tp1: TypeVar, tp2: Type, subst: Substitution) =
